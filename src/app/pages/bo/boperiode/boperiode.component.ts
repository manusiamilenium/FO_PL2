...
export class BoperiodeComponent implements OnInit {
  ...
  loadTransaction() {
    this.http.get("https://pelaporanpliiapi.azurewebsites.net/api/TransaksiLelang/P2PK", this.api.generateHeader()).subscribe((result: any) => {
      if (result.data) {
  ...
  onKirim(idtrans) {
    if (confirm("Apakah anda yakin ingin Buka Akses?")) {
      console.log(idtrans);
      this.http.put("https://pelaporanpliiapi.azurewebsites.net/api/TransaksiLelang/P2PK/BukaAkses?id=" + idtrans, null, this.api.generateHeader()).subscribe(data => {
